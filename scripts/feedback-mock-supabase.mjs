...
const ateliers = [
  ...
  },
  {
    ...
    description: "Atelier passé. Bilan disponible.",
    date_atelier: isoDay(-7),
    heure_debut: "20:00",
    heure_fin: "21:15",
    duree: 75,
...
const EMBED_RESOLVERS = {
  ...
  inscriptions: (row) => {
    const ins = FAKE_DATA.inscriptions.find((i) => i.id === row.inscription_id) ?? null;
    if (!ins) return null;
    // Pour /admin/avis : on charge aussi les ateliers liés via inscriptions.atelier_id
    return {
      ...ins,
      ateliers: FAKE_DATA.ateliers.find((a) => a.id === ins.atelier_id) ?? null,
    };
  },
};
