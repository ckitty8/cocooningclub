...
const MembrePremiumLayout = ({ children }: { children: ReactNode }) => {
  ...
  const isActive = (href: string, end?: boolean) =>
    end ? location.pathname === href : location.pathname === href || location.pathname.startsWith(href + "/");
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      <div className="p-6 border-b">
        ...
        <p className="text-xs text-amber-600 mt-1 tracking-wide uppercase flex items-center gap-1">
          <Crown className="w-3 h-3" /> Espace Premium
        </p>
      </div>
      ...
      <div className="p-4 border-t">
        <Link
          to="/espace-membre-premium/mon-compte"
          onClick={() => setSidebarOpen(false)}
        ...
        >
          {profile?.url_avatar ? (
            ...
            <div className="w-8 h-8 rounded-full bg-primary/20 flex items-center justify-center text-primary font-semibold text-sm shrink-0">
              {(profile?.prenom?.[0] ?? profile?.email?.[0] ?? "M").toUpperCase()}
            </div>
          )}
...
export default MembrePremiumLayout;
